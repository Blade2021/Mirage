...
public class ListToons extends Command {
    ...
    public void dispatch(User sender, MessageChannel channel, Message message, String content, GuildMessageReceivedEvent event) {
        ...
        switch(args[0]){
            case "guild":
                players = MirageApplication.playerService.getPlayersByGuild(args[1]);
                break;
            case "all":
                players = MirageApplication.playerService.getPlayers();
                break;
            default:
                players = MirageApplication.playerService.getPlayersByUID(sender.getIdLong());
                break;
        }
        ...
        if(players.size() > 0) {
            ...
            reply(event, "No players found with that criteria");
        }
    }
